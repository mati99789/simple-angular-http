...
export class PlacesService extends BaseApiService {
  ...
  putPlace(id: string): Observable<Place> {
    ...
    const placeToAdd = this.availablePlacesCache.find(
      (place) => place.id === id
    );
    ...
    return this.put<{ userPlaces: Place }>('user-places', { placeId: id }).pipe(
      ...
      catchError((err) => {
        ...
        if (placeToAdd) {
          const currentUserPlaces = this.userPlacesSubject.value;
          const updatedUserPlaces = currentUserPlaces.filter(
            (place) => place.id !== id
          );
          this.userPlacesSubject.next(updatedUserPlaces);
        }
  ...
  deletePlace(id: string): Observable<Place[]> {
    ...
    const currentUserPlace = this.userPlacesSubject.value.some((place) => place.id === id);
    
    if(currentUserPlace) {
      const updatedUserPlaces = this.userPlacesSubject.value.filter((place) => place.id !== id);
      
      this.userPlacesSubject.next(updatedUserPlaces);
    }
    
    return this.delete<{ userPlaces: Place[] }>(`user-places/${id}`).pipe(
      map((repos) => repos.userPlaces),
      tap((places) => {
        this.userPlacesSubject.next(places);
      }),
      catchError((err) => {
       
        this.errorService.showError('Error remove place');

        return throwError(() => {
          new Error('Error remove place');
        });
      })
    );
  }
